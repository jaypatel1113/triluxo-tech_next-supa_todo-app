import axios from "axios";
import type { BackendDataType, BackendResponse } from "~/types/services/api_requests";
// import type { AxiosResponse, AxiosError } from "axios";

// Define a generic function to send data to the backend
export const delete_data = async <T>(data: { id: T; route: BackendDataType<T>['route'] }): Promise<BackendResponse<T>> => {
    try {
        const { id, route } = data;
        const payload = { id }
        
        const routeWithoutS = route.endsWith('s') ? route.slice(0, -1) : route;

        const response = await axios.post(`/api/${routeWithoutS}`, payload);

        return response.data as BackendResponse<T>;;
    } catch (error) {
        if (axios.isAxiosError(error)) {
            console.error("Error in axios error:", error);
        } else {
            console.error("Error in send data:", error);
        }
        throw error;
    }
};